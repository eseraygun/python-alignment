...
class SequenceAlignment(object):
	...
	def quality(self):
		return self.score, self.percentIdentity(), self.percentSimilarity(), -self.percentGap()
...
class SequenceAligner(object):
	...
	def emptyAlignment(self, first, second):
		return SequenceAlignment(
			EncodedSequence(len(first) + len(second), id=first.id), # pre-allocate
			EncodedSequence(len(first) + len(second), id=second.id), # pre-allocate
		)
...
class GlobalSequenceAligner(SequenceAligner):
	...
	def computeAlignmentMatrix(self, first, second):
		...
		for i in xrange(1, m):
			for j in xrange(1, n):
				# Match elements.
				ab = f[i - 1, j - 1] + self.scoring(first[i - 1], second[j - 1])
	...
	def backtrace(self, first, second, f):
		...
		self.backtraceFrom(first, second, f, m - 1, n - 1, alignments, alignment)
		return alignments
	...
	def backtraceFrom(self, first, second, f, i, j, alignments, alignment):
		if i == 0 or j == 0:
			...
			if c == p + self.scoring(a, b):
				...
				self.backtraceFrom(first, second, f, i - 1, j - 1, alignments, alignment)
				alignment.pop()
			if i == m - 1:
				if c == y:
					self.backtraceFrom(first, second, f, i, j - 1, alignments, alignment)
			...
			if j == n - 1:
				if c == x:
					self.backtraceFrom(first, second, f, i - 1, j, alignments, alignment)
...
class LocalSequenceAligner(SequenceAligner):
	...
	def backtrace(self, first, second, f):
		...
		for i in xrange(m):
			for j in xrange(n):
				if f[i, j] >= minScore:
					self.backtraceFrom(first, second, f, i, j, alignments, alignment)
		return alignments
